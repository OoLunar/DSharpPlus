...
public sealed record InternalButtonComponent : IInternalMessageComponent
{
    ...
    /// </summary>
    /// <remarks>
    /// Non-link buttons must have a <see cref="CustomId"/>, and cannot have a url. Link buttons must have a url, 
    /// and cannot have a <see cref="CustomId"/>. Link buttons do not send an <see cref="InternalInteraction"/> to 
    ...
    /// </summary>
    /// <remarks>
    /// <see cref="CustomId"/> must be unique per component; multiple buttons on the same message must not 
    /// share the same <see cref="CustomId"/>. This field is a string of max 100 characters, and can be used 
